...
def check_subdirs(cube_root: Path):
    ...
    with os.scandir(cube_root) as scan:
        for entry in scan:
            if entry.is_dir() and entry.name not in allowed_subdirs and not rx_tile_id.match(entry.name):
...
def product_files(cube_root: Path,
    ...
    """Fast reading of file information, returned in a JSON-izable data structure"""
            ...
            for e in scan:
                if e.is_file():
                    if match := pattern.match(e.name):
                        infos = {'path': e.path}
                        infos.update({k:match.group(k) for k in pattern.groupindex.keys()})
                        stat = e.stat()
                        infos['t_c'] = datetime.fromtimestamp(stat.st_ctime).isoformat()
                        infos['t_m'] = datetime.fromtimestamp(stat.st_mtime).isoformat()
                        _results.append(infos)
        return _results

    results = []
    tLast = t0 = datetime.now()
    n_folders = 0

    with tqdm(total=len(tile_dirs)) as pbar:
        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            futures = {executor.submit(read_file_information, tile_dir, pattern): i for i, tile_dir in enumerate(tile_dirs)}
...
prod_cube_l71 = product_json(path_lnd071_json, config.DIR_ARD_CUBE / 'LND07_2021')
prod_cube_l72 = product_json(path_lnd072_json, config.DIR_ARD_CUBE / 'LND07_2021' / 'LND07_2021')
