...
def download_satellite_data(
    ...
    for i, product in enumerate(results[:max_results]):
        print(f"Downloading {i+1}/{min(len(results), max_results)}: {product.properties['title']}")
        try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python download_satellite.py [sentinel|landsat] [bbox_min_lon bbox_min_lat bbox_max_lon bbox_max_lat]")
        print("Example: python download_satellite.py sentinel 2.2 48.8 2.5 49.0")
        sys.exit(1)
