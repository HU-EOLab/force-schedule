...
DATACUBE_DIR=/data/Dagobah/dc
FORCE_TEMP_DIR=/var/tmp/force-temp

# Update FORCE each evening
# 20:00 PM each day
0 20 * * * $DATACUBE_DIR/force-schedule/bash/update-force.sh

# update landsatlinks each evening
# 20:05 PM each day
5 20 * * * $DATACUBE_DIR/force-schedule/bash/update-landsatlinks.sh

# Update Google Cloud Storage metadata, and
# download all Sentinel-2 images that were not already processed to ARD
# 21:00 PM each day
0 21 * * * $DATACUBE_DIR/force-schedule/bash/ingest-sentinel2.sh

# generate Landsat links for images that were not already processed to ARD
# download images that were not already downloaded
# extract the images
# 23:00 PM each day
0 23 * * * $DATACUBE_DIR/force-schedule/bash/ingest-landsat.sh

# update MODIS water vapor database
# 23:00 PM each day
30 23 * * * $DATACUBE_DIR/force-schedule/bash/update-wvdb.sh

# create force-temp folder = DIR_TEMP in param/ard/germany-operational*.prm files
...
0 1 * * 1 mkdir -p $FORCE_TEMP_DIR && chown $(id -u):datacube $FORCE_TEMP_DIR && $DATACUBE_DIR/force-schedule/bash/process-sentinel2.sh && $DATACUBE_DIR/force-schedule/bash/process-landsat.sh
